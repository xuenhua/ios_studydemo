...
func  getPicturesArray(name:[String],type:[String],imageName:[String],country:[String],favstateArray:[Bool],text:[String]) -> [MyPicture]{
    ...
    for i in 0 ... pic_count-1 {
        let picture = MyPicture(name: name[i], type: type[i], imageName: imageName[i], country: country[i], favstate: favstateArray[i],text:text[i])
...
func  getPicturesArray2(dataset:MyDataSet) -> [MyPicture]{
    ...
    var pictures=[MyPicture]()
    for i in 0 ... pic_count-1 {
        let picture = MyPicture(name: dataset.name[i], type: dataset.type[i], imageName: dataset.imagename[i], country: dataset.country[i], favstate: dataset.favstateArray[i],text:dataset.text[i])
        pictures.append(picture)
    }
    return pictures
}
